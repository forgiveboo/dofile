...
$.get(chrome.extension.getURL('/chrome/inject/inspect-overlay.html'), function (overlay) {
	$.get(chrome.extension.getURL('/chrome/inject/inspect-toolbar.html'), function (toolbar) {
		overlayTpl = overlay;
		toolbarTpl = toolbar;
...
$('body').on('click', 'amino-inspect-toolbar-list-item.side', function (event) {
	event.preventDefault();
	event.stopPropagation();
	$(this).closest('amino-inspect-toolbar').toggleClass('left').toggleClass('right');
	$(this).children().toggleClass('hide');
});
$('body').on('click', 'amino-inspect-toolbar-list-item.selectors', function (event) {
	event.preventDefault();
	event.stopPropagation();
...
$('body').on('click', 'amino-inspect-toolbar-list-item.drops', function (event) {
	$('amino-inspect-toolbar-list-item').removeClass('active');
	$(this).addClass('active');
	$('.amino-scan-content.selectors').addClass('hide');
	$('.amino-scan-content.drops').removeClass('hide');
...
function attachHandlers (overlayTpl) {
	$(document).on('mouseover',  function (event) {
		event.stopPropagation();
		if (!$(event.target).hasClass('amino-scan-overlay') && !($.contains($('.amino-inspect-toolbar')[0], $(event.target)[0])) && !$(event.target).hasClass('amino-inspect-toolbar') && selectorMode && !dropsMode) {
	...
	$(document).on('click', function (event) {
		if (!$(event.target).hasClass('amino-scan-overlay') && !($.contains($('.amino-inspect-toolbar')[0], $(event.target)[0])) && !$(event.target).hasClass('amino-inspect-toolbar') && dropsMode) {
			event.preventDefault();
			event.stopPropagation();
	...
	$(document).on('mousemove', function (event) {
		event.stopPropagation();
		if (!$(event.target).hasClass('amino-scan-overlay') && !($.contains($('.amino-inspect-toolbar')[0], $(event.target)[0])) && !$(event.target).hasClass('amino-inspect-toolbar') && typeof context !== "undefined" && dropsMode) {
			const p = context.getImageData(event.clientX, event.clientY, 1, 1).data;
...
function parseStyles () {
	...
	return Promise.resolve().then(() => {
		...
		const ruleRegExp = /([a-zA-Z0-9.\-_:,\s#]*)\s{([a-zA-Z0-9.\-_:\,\s;\!#\)\(]*)}/gm;
		for (let style of styles) {
			let match;
			while((match = ruleRegExp.exec(style.innerText)) !== null) {
				if (!match[1].match('::')) {
					$(match[1].trim()).attr('amino-styles', ($(match[1].trim()).attr('amino-styles') || '') + match[2].trim().replace(/ +(?= )/g,''));
				}
			}
		}
	});
...
function resetCopyText () {
	...
	$('.amino-scan-overlay').find('.click-to-copy').children().each(function(key) {
		if (key === 0) {
